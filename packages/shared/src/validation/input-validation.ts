 ...
 * Provides cross-runtime sanitization helpers for XSS, SQL injection, and other attacks.
 *
 * Runtime-specific wrappers should configure logging via `setInputValidationLogger`.
 */

import type { LogMetadata } from "../types.ts";

type WarnLogger = (message: string, metadata?: LogMetadata) => void;

interface InputValidationLogger {
  warn: WarnLogger;
}

const defaultWarn: WarnLogger = (message, metadata) => {
  // Minimal console fallback to avoid introducing structured logger dependencies.
  if (metadata) {
...
let logger: InputValidationLogger = { warn: defaultWarn };

export function setInputValidationLogger(
  customLogger: Partial<InputValidationLogger> | null | undefined,
): void {
  logger = {
    warn: customLogger?.warn ?? defaultWarn,
  };
}
...
export { sanitizeSearchQuery } from "../utils/sanitizer-util.ts";
