...
export function buildFacebookLoginUrl(): string {
  ...
  const FB_REDIRECT_URI = import.meta.env.VITE_OAUTH_CALLBACK_URL ||
  ...
  return `https://www.facebook.com/v23.0/dialog/oauth?client_id=${FB_APP_ID}&redirect_uri=${
    encodeURIComponent(FB_REDIRECT_URI)
  }&scope=${FB_SCOPES}&state=${encodeURIComponent(currentOrigin)}`;
}
...
export async function handleOAuthRedirect(): Promise<void> {
  try {
    ...
    if (success) {
      alert(
        `Successfully connected ${pagesCount} Facebook page(s)! Synced ${eventsCount} events.`,
      );
      window.history.replaceState({}, "", "/"); // clean our browser's url
    ...
    if (code) {
      ...
      const callbackUrl = `${window.location.origin}/api/oauth-callback?code=${
      ...
      alert(
        `Successfully connected ${
          data.pages_count || 0
        } Facebook page(s)! Synced ${data.events_count || 0} events.`,
      );
      window.history.replaceState({}, "", "/");
