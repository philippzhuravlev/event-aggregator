...
import { handleHealthCheck, performHealthCheck } from "./index.ts";
import type { HealthCheckResponse } from "./schema.ts";

function createSupabaseClientMock(options?: {
  shouldFail?: boolean;
  pages?: Array<{ page_id: number; token_expiry?: string; token_status: string }>;
}) {
  ...
  const pagesData = pages.length > 0 ? pages : [];
...
Deno.test("handleHealthCheck returns valid response structure", async () => {
  ...
  try {
    ...
    const payload: HealthCheckResponse = await response.json();
    
    // Validate response structure
    assertExists(payload.system);
    assertExists(payload.tokens);
    assertExists(payload.overall);
    assertExists(payload.alerts);
...
Deno.test("performHealthCheck handles database errors", async () => {
  ...
  assertExists(result.system.error);
});
